...
func TestChannelManagerMoreGoFinish(_ *testing.T) {
	wList := []workJob{}
	for i := 1; i <= 10; i++ {
		// 如果不初始化会阻塞
...
// 无关闭无泄漏，会回收
func TestGoManyChannelTest(*testing.T) {
...
func TestSelectDefault(*testing.T) {
...
func TestJudgeChannelClose(*testing.T) {
	cInt := make(chan int, 0)
	isClose := func(a chan int) bool {
		select {
		case <-a:
...
func TestCloseChannel(*testing.T) {
	cInt := make(chan int, 0)
...
func TestGoChannelTest(*testing.T) {
...
func TestCancelByContext(*testing.T) {
	// gin 的平滑关闭可以这么理解
