...
func runPicTool(_ *cobra.Command, _ []string) {
	runPicTool3()
	arms.DirExistOrCreate("storage/pic/")
	const S = 512
	const S2 = 256
	dc := gg.NewContext(S, S)
	maxT := 3
	...
	logger.ErrIf(dc.SavePNG("storage/pic/background.png"))
	dc = gg.NewContext(S2, S2)
	dc.DrawCircle(S2/2, S2/2, S2/2)
	dc.SetRGB(255, 255, 255)
	dc.SetColor(color.White)
	dc.Fill()
	logger.ErrIf(dc.SavePNG("storage/pic/water.png"))
	...
	outputRoot := config.GetString("PIC_OUTPUT_PATH", "./storage/pic_output")
	for i := 1; i <= 100; i++ {
		...
		dc.Fill()
		logger.ErrIf(dc.SavePNG(outputRoot + "out" + cast.ToString(time.Now().Format("2006_01_02_15_04_05")) + cast.ToString(i) + ".png"))
		fmt.Println("完成")
	}
}
